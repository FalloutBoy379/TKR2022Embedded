...
void loop() 
{
  int xsum = analogRead(xlaser);
  //int ycor = yfilter();
  
  /*xcur[lasercount] = analogRead(xlaser);
  ycur[lasercount]= analogRead(ylaser);
  lasercount ++;
  if (lasercount > sizeof(xcur) || lasercount > sizeof(ycur))
  {
    lasercount = 0;
  }
  for (int i = 0; i <= sizeof(xcur); i++)
  {
    xsum += xcur[i];
    ysum += ycur[i];
  }
  xsum = xsum/sizeof(xcur);
  ysum = ysum/sizeof(ycur);
  */
...
bool drive(int x, int y, int w) {
  ...
  M1Speed = map(M1Speed, -127, 127, 60, 67);
  M2Speed = map(M2Speed, -127, 127, 60, 67);
  M3Speed = map(M3Speed, -127, 127, 60, 67);
  /*Serial.print("\t");
